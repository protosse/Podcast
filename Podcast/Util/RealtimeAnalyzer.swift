...
class RealtimeAnalyzer {
    ...
    private func fft(_ buffer: AVAudioPCMBuffer) -> [[Float]] {
        ...
        if isInterleaved {
            ...
            for i in 0 ..< channelCount {
                ...
                let floatPointer = UnsafeMutablePointer<Float>.allocate(capacity: channelData.count)
                floatPointer.initialize(from: &channelData, count: channelData.count)
                channelsTemp.append(floatPointer)
            }
            let pointerFloatPointer = UnsafeMutablePointer<UnsafeMutablePointer<Float>>.allocate(capacity: channelsTemp.count)
            pointerFloatPointer.initialize(from: &channelsTemp, count: channelsTemp.count)
            channels = UnsafePointer(pointerFloatPointer)
        }
        ...
        for i in 0 ..< channelCount {
            ...
            let realpPointer = UnsafeMutablePointer<Float>.allocate(capacity: realp.count)
            realpPointer.initialize(from: &realp, count: realp.count)

            let imagpPointer = UnsafeMutablePointer<Float>.allocate(capacity: imagp.count)
            imagpPointer.initialize(from: &imagp, count: imagp.count)

            var fftInOut = DSPSplitComplex(realp: realpPointer, imagp: imagpPointer)
    ...
    private func findMaxAmplitude(for band: (lowerFrequency: Float, upperFrequency: Float), in amplitudes: [Float], with bandWidth: Float) -> Float {
        ...
        if endIndex > startIndex {
            ...
            return amplitudes[endIndex ... startIndex].max()!
        }
    }
