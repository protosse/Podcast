...
public class AudioPlayer: NSObject {
    ...
    public internal(set) var currentItem: AudioItem? {
        didSet {
            if let currentItem = currentItem {
                ...
                if reachability.connection != .unavailable || info.url.ap_isOfflineURL {
                    state = .buffering
    ...
    func setAudioSession(active: Bool) {
            ...
            if #available(iOS 11.0, tvOS 11.0, *) {
                _ = try? AVAudioSession.sharedInstance().setCategory(.playback, mode: .default, policy: .longFormAudio, options: [])
            } else {
